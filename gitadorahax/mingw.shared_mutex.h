  ...
  public:
    ...
    void lock_shared(void) {
        ...
        do {
            ...
            if (mCounter.compare_exchange_weak(expected, expected + 1, std::memory_order_acquire,
    ...
    bool try_lock_shared(void) {
        ...
        if (expected + 1 == kWriteBit)
            ...
            return mCounter.compare_exchange_strong(
    ...
    bool try_lock(void) {
        ...
        bool ret = mCounter.compare_exchange_strong(expected, kWriteBit, std::memory_order_acquire,
                                                    std::memory_order_relaxed);
#if STDMUTEX_RECURSION_CHECKS
        if (ret)
            mOwnerThread.setOwnerAfterLock(self);
    ...
    native_handle_type native_handle(void) { return this; }
};
...
template <class Mutex> class shared_lock {
  ...
  public:
    ...
    shared_lock(shared_lock<Mutex> &&other) noexcept : mMutex(other.mutex_), mOwns(other.owns_) {
        other.mMutex = nullptr;
        other.mOwns = false;
    }

    explicit shared_lock(mutex_type &m) : mMutex(&m), mOwns(true) { mMutex->lock_shared(); }
    ...
    shared_lock(mutex_type &m, try_to_lock_t) : mMutex(&m), mOwns(m.try_lock_shared()) {}
    ...
    //  Observers
    mutex_type *mutex(void) const noexcept { return mMutex; }

    bool owns_lock(void) const noexcept { return mOwns; }

    explicit operator bool() const noexcept { return owns_lock(); }
};

template <class Mutex> void swap(shared_lock<Mutex> &lhs, shared_lock<Mutex> &rhs) noexcept {
    lhs.swap(rhs);
}
#endif //  C++11
} //  Namespace mingw_stdthread
