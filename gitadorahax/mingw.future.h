...
template <bool> struct FutureStatic {
...
template <bool b>
std::vector<std::pair<mutex, condition_variable>>
    FutureStatic<b>::sync_pool(thread::hardware_concurrency() * 2 + 1);

struct FutureStateBase {
    inline mutex &get_mutex(void) const { return FutureStatic<true>::get_mutex(this); }
    ...
    FutureStateBase(Type t) noexcept : mReferences(0), mType(t) {}
...
struct FutureBase : public FutureStatic<true> {
    ...
    mutex &get_mutex(void) const { return FutureStatic<true>::get_mutex(mState); }
    condition_variable &get_condition_variable(void) const {
        return FutureStatic<true>::get_condition_variable(mState);
    }

    FutureBase(FutureStateBase *ptr) noexcept : mState(ptr) {}

    FutureBase(FutureBase &&source) noexcept : mState(source.mState) { source.mState = nullptr; }

    ~FutureBase(void) { release(); }

    FutureBase(FutureBase const &) = delete;
    FutureBase &operator=(FutureBase const &) = delete;

    bool valid(void) const noexcept { return mState != nullptr; }
    ...
    future_status wait_for(std::chrono::duration<Rep, Period> const &dur) const {
        ...
        if (get_condition_variable().wait_for(lock, dur, [this](void) -> bool {
                return mState->mType.load(std::memory_order_relaxed) & kReadyFlag;
            }))
...
template <class T> struct FutureState : public FutureStateBase {
    //    The state never needs more than one of these at any one time, so don't
    //  waste space or allocation time.
    union {
        struct {
        } mUndecided; //  Included to make the active member unambiguous.
        T mObject;
        std::exception_ptr mException;
        std::function<void(void)> mFunction;
    };

    FutureState(void) noexcept : FutureStateBase(Type::kUndecided), mUndecided() {}

    FutureState(std::function<void(void)> &&deferred_function)
        : FutureStateBase(Type::kDeferred), mFunction(std::move(deferred_function)) {}

    void deallocate_this(void) noexcept override { delete this; }

    template <class Arg> void set_value(Arg &&arg) {
    ...
    template <class Arg> void set_exception(Arg &&arg) {
        assert(!(mType.load(std::memory_order_relaxed) & Type::kSetFlag));
    ...
    template <class Arg> void set_value(Arg &&arg, bool) {
        assert(!(mType.load(std::memory_order_relaxed) & Type::kSetFlag));
        new (&mObject) T(std::forward<Arg>(arg));
        mType.store(Type::kValue, std::memory_order_release);
    }
    template <class Arg> void set_exception(Arg &&arg, bool) {
        assert(!(mType.load(std::memory_order_relaxed) & Type::kSetFlag));
        new (&mException) std::exception_ptr(std::forward<Arg>(arg));
...
template <class T, class Alloc> struct FutureStateAllocated : public FutureState<T> {
...
template <class T> class future : mingw_stdthread::detail::FutureBase {
    typedef mingw_stdthread::detail::FutureState<T> state_type;
    future(state_type *ptr) noexcept : FutureBase(ptr) {}
  ...
  public:
    ...
    future(void) noexcept : FutureBase(nullptr) {}
    ...
    future(future<T> &&source) noexcept : FutureBase(std::move(source)) {}
    ...
    shared_future<T> share(void) noexcept { return std::move(shared_future<T>(std::move(*this))); }
...
template <class T> class shared_future : future<T> {
  ...
  public:
    ...
    shared_future(shared_future<T> &&source) noexcept : future<T>(std::move(source)) {}
    ...
    shared_future(future<T> &&source) noexcept : future<T>(std::move(source)) {}
...
template <class T> class promise : mingw_stdthread::detail::FutureBase {
    bool mRetrieved;
    typedef mingw_stdthread::detail::FutureState<T> state_type;
    ...
    void make_ready_at_thread_exit(void) {
        ...
        if (!success)
            throw std::runtime_error(
                "MinGW STD Threads library failed to make a promise ready after thread exit.");
        ...
        try {
            ...
            });
            {
                ...
                ptr->get_condition_variable().wait(
                    guard, [&handle_handled](void) -> bool { return handle_handled; });
            }
    ...
    template <class U> future<U> make_future(void) {
        if (!valid())
            throw future_error(future_errc::no_state);
        if (mRetrieved)
  ...
  public:
    ...
    //  futures).
    promise(void) : FutureBase(new state_type()), mRetrieved(false) {}

    template <typename Alloc>
    promise(std::allocator_arg_t, Alloc const &alloc) : FutureBase(nullptr), mRetrieved(false) {
    ...
    future<T> get_future(void) { return make_future<T>(); }
    ...
    void set_value(T const &value) {
        {
            ...
            //    {detail::FutureStatic<true>::get_mutex(state_ptr_.get()); };
...
template <class T> class future<T &> : future<void *> {
    typedef future<void *> Base;
    template <class U> friend class shared_future;
    template <class U> friend class promise;

    future(typename Base::state_type *state) : Base(state) {}
  ...
  public:
    ...
    inline T &get(void) const { return *static_cast<T *>(Base::get()); }

    shared_future<T &> share(void) noexcept;
};

template <class T> class shared_future<T &> : shared_future<void *> {
  ...
  public:
    ...
    shared_future<T&> & operator= (shared_future<T&> const & source) noexcept(__cplusplus >=
    201703L) = default;*/

    shared_future(future<T &> &&source) noexcept : Base(std::move(source)) {}
...
template <class T> shared_future<T &> future<T &>::share(void) noexcept {
    return std::move(shared_future<T &>(std::move(*this)));
}

template <class T> class promise<T &> : private promise<void *> {
  ...
  public:
    ...
    template <typename Alloc>
    promise(std::allocator_arg_t arg, Alloc const &alloc) : Base(arg, alloc) {}
    ...
    inline future<T &> get_future(void) { return Base::template make_future<T &>(); }

    void swap(promise<T &> &other) noexcept { Base::swap(other); }
};
...
template <> class future<void> : future<mingw_stdthread::detail::Empty> {
    typedef mingw_stdthread::detail::Empty Empty;
    template <class U> friend class shared_future;
    template <class U> friend class promise;

    future(future<Empty>::state_type *state) : future<Empty>(state) {}
  ...
  public:
    ...
    void get(void) const { future<Empty>::get(); }

    shared_future<void> share(void) noexcept;
};

template <> class shared_future<void> : shared_future<mingw_stdthread::detail::Empty> {
    typedef mingw_stdthread::detail::Empty Empty;
  ...
  public:
    ...
    void get(void) const { shared_future<Empty>::get(); }
    ...
    shared_future<void> &operator=(shared_future<void> const &source) noexcept(__cplusplus >=
                                                                               201703L) = default;

    shared_future(future<void> &&source) noexcept : shared_future<Empty>(std::move(source)) {}
...
template <> class promise<void> : private promise<mingw_stdthread::detail::Empty> {
    typedef mingw_stdthread::detail::Empty Empty;
  ...
  public:
    ...
    promise(void) = default;
    template <typename Alloc>
    promise(std::allocator_arg_t arg, Alloc const &alloc) : promise<Empty>(arg, alloc) {}

    inline void set_value(void) { promise<Empty>::set_value(Empty()); }

    inline void set_value_at_thread_exit(void) {
        promise<Empty>::set_value_at_thread_exit(Empty());
    }

    inline future<void> get_future(void) { return promise<Empty>::template make_future<void>(); }

    void swap(promise<void> &other) noexcept { promise<Empty>::swap(other); }
};

template <class T> void swap(promise<T> &lhs, promise<T> &rhs) noexcept { lhs.swap(rhs); }

template <class T, class Alloc> struct uses_allocator<promise<T>, Alloc> : std::true_type {};

} // namespace mingw_stdthread
namespace mingw_stdthread {
namespace detail {
template <class Ret> struct StorageHelper {
    template <class Func, class... Args>
    static void store_deferred(FutureState<Ret> *state_ptr, Func &&func, Args &&... args) {
        try {
            state_ptr->set_value(mingw_stdthread::detail::invoke(std::forward<Func>(func),
...
template <class Ref> struct StorageHelper<Ref &> {
    template <class Func, class... Args>
    static void store_deferred(FutureState<void *> *state_ptr, Func &&func, Args &&... args) {
        try {
            ...
            Ref &rf = mingw_stdthread::detail::invoke(std::forward<Func>(func),
                                                      std::forward<Args>(args)...);
...
template <> struct StorageHelper<void> {
    template <class Func, class... Args>
...
[[nodiscard]]
    ...
    /*#if (__cplusplus < 201703L)
      typedef std::result_of<std::decay<Function>::type(std::decay<Args>::type...)>::type
    result_type; #else typedef std::invoke_result_t<std::decay_t<Function>, std::decay_t<Args>...>
    result_type; #endif*/
    ...
    /*if ((policy & std::launch::async) == std::launch::async)
      ...
      state_ptr = new state_type
    (std::function<result_type(void)>(std::bind(std::forward<Function>(f),
    std::forward<Args>(args)...)));*/
    ...
    if ((policy & std::launch::async) == std::launch::async) {
        ...
        mingw_stdthread::thread t(
            [](decltype(ooptr) ptr, typename std::decay<Function>::type f2,
               typename std::decay<Args>::type... args2) {
        ...
        std::shared_ptr<Packed> bound{
            new Packed{std::bind(std::forward<Function>(f), std::forward<Args>(args)...), nullptr}};
