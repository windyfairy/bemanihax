...
class condition_variable_any {
  ...
  public:
    ...
    condition_variable_any()
        : mMutex(), mNumWaiters(0), mSemaphore(CreateSemaphore(NULL, 0, 0xFFFF, NULL)),
  ...
  protected:
    template <class M> bool wait_impl(M &lock, DWORD timeout) {
        {
  ...
  public:
    template <class M> void wait(M &lock) { wait_impl(lock, INFINITE); }
    template <class M, class Predicate> void wait(M &lock, Predicate pred) {
        while (!pred()) {
            wait(lock);
        };
    }
    ...
    void notify_all() noexcept {
        lock_guard<recursive_mutex> lock(
            mMutex); // block any further wait requests until all current waiters are unblocked
    ...
    bool wait_until(M &lock, const std::chrono::time_point<Clock, Duration> &abs_time,
                    Predicate pred) {
        while (!pred()) {
...
class condition_variable : protected condition_variable_any {
  ...
  public:
    ...
    void wait(unique_lock<mutex> &lock) { base::wait(lock); }
    ...
    bool wait_for(unique_lock<mutex> &lock, const std::chrono::duration<Rep, Period> &rel_time,
                  Predicate pred) {
...
class condition_variable {
  protected:
    ...
    template <typename MTX> inline static void before_wait(MTX *pmutex) {
        pmutex->mOwnerThread.checkSetOwnerBeforeUnlock();
    }
    template <typename MTX> inline static void after_wait(MTX *pmutex) {
    ...
    bool wait_impl(unique_lock<xp::mutex> &lock, DWORD time) {
        static_assert(
            std::is_same<typename xp::mutex::native_handle_type, PCRITICAL_SECTION>::value,
...
use native Win32 critical section objects.");
        ...
        before_wait(pmutex);
        BOOL success = SleepConditionVariableCS(&cvariable_, pmutex->native_handle(), time);
        after_wait(pmutex);
    ...
    bool wait_unique(windows7::mutex *pmutex, DWORD time) {
        before_wait(pmutex);
        BOOL success = SleepConditionVariableSRW(native_handle(), pmutex->native_handle(), time, 0);
        after_wait(pmutex);
        return success;
    }
  ...
  public:
    ...
    native_handle_type native_handle(void) { return &cvariable_; }

    condition_variable(void) : cvariable_() { InitializeConditionVariable(&cvariable_); }
    ...
    void notify_one(void) noexcept { WakeConditionVariable(&cvariable_); }

    void notify_all(void) noexcept { WakeAllConditionVariable(&cvariable_); }

    void wait(unique_lock<mutex> &lock) { wait_impl(lock, INFINITE); }
...
class condition_variable_any : protected condition_variable {
  protected:
    ...
    template <class L> bool wait_impl(L &lock, DWORD time) {
...
to be 0. There is a conflict with CONDITION_VARIABLE_LOCKMODE_SHARED.");
    ...
    bool wait_impl(shared_lock<native_shared_mutex> &lock, DWORD time) {
        native_shared_mutex *pmutex = lock.release();
        BOOL success = SleepConditionVariableSRW(base::native_handle(), pmutex->native_handle(),
                                                 time, CONDITION_VARIABLE_LOCKMODE_SHARED);
  ...
  public:
    ...
    template <class L> void wait(L &lock) { wait_impl(lock, INFINITE); }

    template <class L, class Predicate> void wait(L &lock, Predicate pred) {
        while (!pred())
            wait(lock);
    }
    ...
    bool wait_for(L &lock, const std::chrono::duration<Rep, Period> &period, Predicate pred) {
        return wait_until(lock, std::chrono::steady_clock::now() + period, std::move(pred));
    }
    ...
    bool wait_until(L &lock, const std::chrono::time_point<Clock, Duration> &abs_time,
                    Predicate pred) {
        while (!pred()) {
            if (wait_until(lock, abs_time) == cv_status::timeout) {
                return pred();
