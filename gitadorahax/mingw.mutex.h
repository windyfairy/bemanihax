...
class recursive_mutex {
  ...
  public:
    ...
    recursive_mutex() noexcept : mHandle() { InitializeCriticalSection(&mHandle); }
    recursive_mutex(const recursive_mutex &) = delete;
    recursive_mutex &operator=(const recursive_mutex &) = delete;
    ~recursive_mutex() noexcept { DeleteCriticalSection(&mHandle); }
    void lock() { EnterCriticalSection(&mHandle); }
    void unlock() { LeaveCriticalSection(&mHandle); }
    bool try_lock() { return (TryEnterCriticalSection(&mHandle) != 0); }
};
...
struct _OwnerThread {
 ...
 detected. Throwing system exception\n");
    ...
    void setOwnerAfterLock(DWORD id) { mOwnerThread.store(id, std::memory_order_relaxed); }
...
#endif
    native_handle_type native_handle(void) { return &mHandle; }
};
} //  Namespace windows7
...
//  Track locking thread for error checking.
  ...
  public:
    ...
    mutex(const mutex &) = delete;
    mutex &operator=(const mutex &) = delete;
    ~mutex() noexcept { DeleteCriticalSection(&mHandle); }
    void lock(void) {
...
//  access-control level as every other variable in the timed_mutex.
  ...
  public:
    ...
    ~recursive_timed_mutex() { CloseHandle(mHandle); }
    void lock() { try_lock_internal(INFINITE); }
    void unlock() {
        using namespace std;
        if (!ReleaseMutex(mHandle))
            throw system_error(make_error_code(errc::resource_deadlock_would_occur));
    }
    bool try_lock() { return try_lock_internal(0); }
...
class timed_mutex : recursive_timed_mutex {
  public:
    ...
    bool try_lock() { return try_lock_for(std::chrono::milliseconds(0)); }
};
#else
